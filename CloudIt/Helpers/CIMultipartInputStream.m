...
@interface CIMultipartElement : NSObject
...
@implementation CIMultipartElement
- (void)updateLength
{
...
@interface CIMultipartInputStream()
...
@implementation CIMultipartInputStream
- (void)updateLength
{
...
-(void)addParts:(NSDictionary *)parts
{
    for (NSString* name in parts) {
        [self addPartWithName:name string:[NSString stringWithFormat: @"%@", [parts objectForKey:name]]];
    }
}

- (void)addPartWithName:(NSString *)name string:(NSString *)string
{
    [self.parts addObject:[[CIMultipartElement alloc] initWithName:name boundary:self.boundary string:string]];
    [self updateLength];
}
- (void)addPartWithName:(NSString *)name data:(NSData *)data
{
    [self.parts addObject:[[CIMultipartElement alloc] initWithName:name boundary:self.boundary data:data contentType:@"application/octet-stream"]];
    [self updateLength];
}
- (void)addPartWithName:(NSString *)name data:(NSData *)data contentType:(NSString *)type
{
    [self.parts addObject:[[CIMultipartElement alloc] initWithName:name boundary:self.boundary data:data contentType:type]];
    [self updateLength];
}
- (void)addPartWithName:(NSString *)name path:(NSString *)path
{
    [self.parts addObject:[[CIMultipartElement alloc] initWithName:name filename:nil boundary:self.boundary path:path]];
    [self updateLength];
}

- (void)addPartWithName:(NSString *)name filename:(NSString *)filename path:(NSString *)path
{
    [self.parts addObject:[[CIMultipartElement alloc] initWithName:name filename:filename boundary:self.boundary path:path]];
    [self updateLength];
}
...
{
    [self.parts addObject:[[CIMultipartElement alloc] initWithName:name filename:filename boundary:self.boundary stream:stream streamLength:streamLength]];
    [self updateLength];
}
